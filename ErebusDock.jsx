...
function WaveformAvatar({ isSpeaking, agentColor }) {
  const colorMap = {
...
function AgentSettingsPanel({ agent, onUpdate, onClose }) {
...
export default function ErebusDock() {
  ...
  const deleteAgent = (id) => {
    ...
    if (activeAgentId === id) {
      const remainingAgent = agents.find((a) => a.id !== id);
      if (remainingAgent) {
        setActiveAgentId(remainingAgent.id);
      }
    }
  };
