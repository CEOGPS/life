...
import Topbar from "@/components/lifeos/Topbar";
...
import AgentDock from "@/components/lifeos/AgentDock";
...
import OAuthConsent from "@/components/lifeos/OAuthConsent";
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const BASE_ALPHA = 0.08;  // faint base dots on black so the red lighting pops on hover
    const HOVER_R = 120;      // generous hover range so it feels responsive
      ...
      for (let x = DOT_SPACING / 2; x < window.innerWidth; x += DOT_SPACING) {
        for (let y = DOT_SPACING / 2; y < window.innerHeight; y += DOT_SPACING) {
          ...
          if (dist < HOVER_R) {
            ...
            ctx.fillStyle = `rgba(255,0,0,${t * 0.55})`;
            ctx.fill();
          }
