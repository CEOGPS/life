...
export default function MapsPanel() {
  ...
  return (
    <PanelLayout
      ...
      icon={<MapPin size={18} />}
      actions={
        <button className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg glass-crimson text-primary text-xs font-display hover:glow-crimson-sm transition-all cursor-pointer">
        ...
        <div className="flex gap-4">
          ...
          <div className="w-56 shrink-0 flex flex-col gap-3">
              ...
              <div className="grid grid-cols-2 gap-1.5">
                {TOOLS.map((t, i) => (
                  <button key={t.label} onClick={() => setActiveTool(i)}
                    className={`flex items-center gap-1.5 px-2 py-2 rounded-lg text-[10px] font-display transition-colors cursor-pointer
            ...
            <div className="glass rounded-xl border border-white/8 p-3 flex-1">
              <div className="flex items-center justify-between mb-2">
                ...
                <button className="text-[9px] text-white/20 hover:text-primary/60 font-display cursor-pointer">+ NEW</button>
              </div>
              <div className="space-y-1.5">
                {SAVED_AREAS.map((a) => (
                  ...
                  >
                    ...
                    <button className="text-white/20 hover:text-primary/60 shrink-0 cursor-pointer"><Trash2 size={9} /></button>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="flex-1 glass rounded-xl border overflow-hidden flex flex-col" style={{ minHeight: 440, borderColor: "oklch(0.55 0.22 20 / 18%)" }}>
            ...
            <div className="flex items-center gap-2 px-3 py-2 border-b" style={{ borderColor: "oklch(0.55 0.22 20 / 12%)", background: "oklch(0.05 0 0 / 80%)" }}>
              <button className="text-white/30 hover:text-white/70 p-1 rounded transition-colors cursor-pointer"><ZoomIn size={13} /></button>
              <button className="text-white/30 hover:text-white/70 p-1 rounded transition-colors cursor-pointer"><ZoomOut size={13} /></button>
              <div className="flex gap-1 ml-2">
                {["Road", "Satellite", "Hybrid"].map((m, i) => (
                  <button key={m} className={`text-[9px] px-2 py-0.5 rounded font-display transition-colors cursor-pointer
            ...
            >
              ...
              <div className="text-center z-10">
                ...
                <button className="px-4 py-2 rounded-lg glass-crimson text-primary text-xs font-display hover:glow-crimson-sm transition-all cursor-pointer">
                  CONNECT MAPS API
                </button>
              </div>
