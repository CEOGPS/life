    ...
    <PanelLayout title="Media" subtitle="Store, organize, and manage all your files" icon={<ImageIcon size={18} />}
      actions={
        <div className="flex gap-2">
          <button className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg glass text-white/50 text-xs font-display hover:text-white/80 transition-all cursor-pointer"><Upload size={12} /> UPLOAD</button>
          <button className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg glass-crimson text-primary text-xs font-display hover:glow-crimson-sm transition-all cursor-pointer"><Plus size={12} /> NEW ALBUM</button>
        </div>
      }
    >
        ...
        <div className="flex-1 flex flex-col gap-3 min-w-0">
          <div className="flex items-center gap-3">
            ...
            <div className="flex gap-1">
              <button onClick={() => setView("grid")} className={`p-1.5 rounded cursor-pointer ${view === "grid" ? "glass-crimson text-primary" : "text-white/30 hover:text-white/60"}`}><Grid3x3 size={13} /></button>
              <button onClick={() => setView("list")} className={`p-1.5 rounded cursor-pointer ${view === "list" ? "glass-crimson text-primary" : "text-white/30 hover:text-white/60"}`}><List size={13} /></button>
            </div>
          </div>
          <div className="flex-1 glass rounded-xl border border-white/8 flex flex-col overflow-hidden">
            <div className="flex-1 flex items-center justify-center">
              <div className="text-center">
                ...
                <button className="mt-3 flex items-center gap-1.5 mx-auto px-4 py-1.5 rounded-lg glass-crimson text-primary text-xs font-display hover:glow-crimson-sm transition-all cursor-pointer">
