...
export default function FinancialAnalytics({ kpis, onUpdateKPI }) {
  const [editingId, setEditingId] = useState(null);
  const [editValue, setEditValue] = useState("");

  const totalRevenue = kpis.find(k => k.id === "revenue")?.value || 0;
  const totalCosts = kpis.find(k => k.id === "costs")?.value || 0;
  const grossProfit = totalRevenue - totalCosts;
  const profitMargin = totalRevenue > 0 ? ((grossProfit / totalRevenue) * 100).toFixed(1) : 0;

  const summary = [
    { label: "Revenue", value: `$${totalRevenue.toLocaleString()}`, color: C.teal },
    { label: "Costs", value: `$${totalCosts.toLocaleString()}`, color: C.orange },
    { label: "Gross Profit", value: `$${grossProfit.toLocaleString()}`, color: C.blue },
    { label: "Margin", value: `${profitMargin}%`, color: C.pink },
  ];
