...
export default function WebsiteAnalytics({ kpis, onUpdateKPI }) {
  const [editingId, setEditingId] = useState(null);
  const [editValue, setEditValue] = useState("");

  const traffic = kpis.find(k => k.id === "traffic")?.value || 0;
  const conversions = kpis.find(k => k.id === "conversions")?.value || 0;
  const bounce = kpis.find(k => k.id === "bounce")?.value || 0;
  const avgTime = kpis.find(k => k.id === "avgTime")?.value || 0;

  const summary = [
    { label: "Monthly Visitors", value: traffic.toLocaleString(), color: C.blue },
    { label: "Conversion Rate", value: `${conversions}%`, color: C.teal },
    { label: "Bounce Rate", value: `${bounce}%`, color: C.orange },
    { label: "Avg Time", value: `${avgTime}s`, color: C.pink },
  ];
